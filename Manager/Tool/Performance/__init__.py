...
class VxTool:
    ...
    def run_internal(self, projections: np.ndarray, algo: str = ReconMethodConstants.FDK,
        ...
        match algo.upper():
            case ReconMethodConstants.FDK:
                self.result = self.runFdk_Internal(projections)
            case ReconMethodConstants.SIRT | "SIRT3D_CUDA":
                self.result = self.runIterative_Internal(
                    projections,
    ...
    def plot_geometry(self, angle_index: int | None = None, show: bool = True, save_path: str | None = None):
        ...
        p = self._param
        vecs = self.getPaddedVectors_Internal(self.getVectors_Internal())
        ...
        for idx in range(n_views):
            source, detector, _, _, corners = self.unpack_view_geometry(
                vecs[idx], half_u_px, half_v_px)
        ...
        # everything the per-view draw needs, gathered once
        scene = {
            "ax": ax,
            "vecs": vecs,
            "half_u_px": half_u_px,
    ...
    # static
    @staticmethod
    def generate_sinogram(projections: np.ndarray, copy: bool = True) -> np.ndarray:
        """
        Reorder a projection stack into the sinogram layout ASTRA expects:
        (angles, rows, cols) -> (rows, angles, cols).

        The transpose alone is only a view, and a non-contiguous one, so by
        default the result is materialised contiguously, which is the layout
        astra.data3d.create needs anyway, and which keeps later in-place steps
        (the redundancy weighting) off the caller's array. Pass copy=False for
        the bare view when you only need to inspect the ordering.
        """
        if projections.ndim != 3:
            raise ValueError("Projections must be a 3D (angles, height, width) stack")

        sinogram = np.transpose(projections, (1, 0, 2))
        return np.ascontiguousarray(sinogram) if copy else sinogram

    @staticmethod
    def unpack_view_geometry(vec_row: np.ndarray, half_u_px: float, half_v_px: float):
        """
        Decode one ASTRA cone_vec row into the pieces that describe a single
        view: source, detector centre, the U and V pixel axes, and the four
        detector corners.

        vec_row is [Sx Sy Sz  Dx Dy Dz  Ux Uy Uz  Vx Vy Vz]; the half extents
        are in pixels, so the corners come out in the same units as the vectors.
        """
        if vec_row.shape[-1] != 12:
            raise ValueError("A cone_vec row must have 12 elements")

        source = vec_row[0:3]
        detector = vec_row[3:6]
        u = vec_row[6:9]
        v = vec_row[9:12]
        corners = np.array([
    ...
    def getPaddedVectors_Internal(self, vecs: np.ndarray) -> np.ndarray:
    ...
    def extrapolate_Internal(self, projections: np.ndarray) -> np.ndarray:
        left_pad = max(0, self._left_pad)
        right_pad = max(0, self._right_pad)
        if left_pad == 0 and right_pad == 0:
            return projections
    ...
    def applyRedundancyWeight_Internal(
        self,
        sino,
        offsetX,
        srcPitch,
    ...
    def prepareAstraData_Internal(self, astra, projections: np.ndarray):
        p = self._param
        vecs = self.getPaddedVectors_Internal(self.getVectors_Internal())

        # Geometry vectors are in mm. ASTRA vol_geom must use the same unit.
        sdd = p.sdd
        voxel_size = p.det_pitch * p.sod / sdd
    ...
    def runFdk_Internal(self, projections: np.ndarray) -> np.ndarray:

        proj_id, vol_id = self.prepareAstraData_Internal(astra, projections)
    ...
    def runIterative_Internal(self,
    ...
    def drawView_Internal(self, scene: dict, idx: int):
        """Render one view of the cone-beam geometry onto the scene's axes."""
        from mpl_toolkits.mplot3d.art3d import Poly3DCollection

        ax = scene["ax"]
        half_u_px = scene["half_u_px"]
        half_v_px = scene["half_v_px"]
        volume_faces = scene["volume_faces"]
        center = scene["center"]
        radius = scene["radius"]

        source, detector, u, v, corners = self.unpack_view_geometry(
            scene["vecs"][idx], half_u_px, half_v_px)
        unit_u = u / np.linalg.norm(u)
        unit_v = v / np.linalg.norm(v)
        ax.clear()
        ax.set_title(f"Current cone-beam setup, view {idx}")
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_zlabel("Z")
    ...
    def onViewChanged_Internal(self, scene: dict, fig, value):
        """Slider callback: redraw at the selected view. Bound with partial."""
        self.drawView_Internal(scene, int(value))
