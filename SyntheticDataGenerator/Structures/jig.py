...
class JigStructure:
    """
    JIG: a 22-sphere calibration/verification test piece for monitoring the
    sphere-centre-distance error SD (VDI/VDE 2617 p.13, VDI/VDE 2630 p.1.3)
    and, paired with a length-standard artefact, the length measuring error
    E of a CT scanner.
    ...
    This is a full-size artefact for a general industrial CT (base+cover
    span ~76 mm) - not to be confused with MicroJigStructure, which is the
    much smaller, non-planar layout sized for a micro-CT/small-FOV cone-beam
    setup.
    ...
    def GetStructure(vol_shape: tuple[int, int, int], voxel_size_mm: float) -> np.ndarray:
        ...
        vol, _ = JigStructure.Build(vol_shape, voxel_size_mm)
        return vol
